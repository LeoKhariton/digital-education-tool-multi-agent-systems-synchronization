...
**Рис. 4.1:** Сетевое управление мультиагентными системами: статические связи (слева) и динамические связи (справа)
...
**Рис. 4.2:** Траектории синхронизированных осцилляторов
...
Агенты имеют одинаковые динамические свойства и описываются моделью пространства состояний
...
**Предположение 4.1** *Предполагается, что агенты (4.2) полностью наблюдаемы, т. е. удовлетворяют критерию Хаутуса*
...
с ${a_{ij}}$ в качестве соответствующего элемента матрицы смежности $\mathbf{A}$ коммуникационного графа. Выходная разность ${y_i}(t) - {y_j}(t)$ возникает в законе управления $C$ при наличии ребра ($j \to i$) в ориентированном графе связи $\vec{\mathcal{G}}$ всей системы.
...
**Рис. 4.3:**Синхронизированные агенты

Диффузный характер контроллера означает, что если узлы синхронизированы, сетевой контроллер $C$ не влияет на поведение агента:
...
Регулятор (4.5) можно записать в терминах матрицы Лапласа $L = ({l_{ij}})$ как
...
Эту модель общей системы $\bar \sum $ можно записать в виде
...
Используя произведение Кронекера (см. приложение 2), эти матрицы можно записать в виде
...
$$\bar C = {I_N} \otimes {c^{\rm{T}}} \tag{4.11}. $$ 
...
В силу предположения 4.1 матрица во второй строке имеет полный ранг, откуда следует, что система (4.9) вполне наблюдаема. Следовательно, выходные траектории агентов могут быть одинаковыми только в том случае, если совпадают траектории состояний и уравнение (4.1) эквивалентно (4.12), что доказывает лемму.
...
*Сетевая система (4.9) синхронна тогда и только тогда, когда матрицы
...
1. Условие синхронизации основано на требовании обращения в нуль ошибок синхронизации $e_i(t)=x_i(t)-x_1(t)$, $(i = 2, 3, \dots, N)$.
...
**1. Представление ошибки синхронизации.**Для выполнения условия синхронизации (4.1) необходимо и достаточно выполнения $N-1$ требований
...
Последнее слагаемое обнуляется благодаря уравнению (2.27), и модель приобретает вид
...
$$\widetilde L_{22}=L_{22}-\mathbb1/l_{12}^T.\tag{4.17}$$
...
В дальнейших исследованиях сначала рассматривается случай, когда матрица $\widetilde L_{22}$ диагонализуема и, следовательно, может быть приведена к диагональной матрице с помощью ($N-1$)-мерной матрицы $\widetilde T_{22}^{-1}$:

$$\widetilde T_{22}\widetilde L_{22}^T\widetilde T_{22}^{-1} = \begin{pmatrix} \lambda_2\{L\} &  &  \\  & \ddots &  \\  &  & \lambda_N\{L\} \end{pmatrix}.\tag{4.18}$$

При таком преобразовании модель приобретает следующий вид:
...
**3. Анализ сходиомсти.**Условие синхронизации (4.15) эквивалентно $\lim_{t \to \infty} \lVert x_i(t) - x_j(t) \rVert = 0, \quad (i = 2,3,\dots,N)$ и требованию асимптотической устойчивости матриц

$$A-\lambda_i\{L\}bc^T, \quad i = 2,3,\dots,N$$

заданных в уравнении (4.13), что доказывает теорему о диагонализуемых матрицах $L$.

Если матрица $\widetilde{L}_{22}$ недиагонализуема, то приведенные выше соображения существенно не меняются. После того как матрица $\widetilde{L}_{22}^T$ была преобразована в жорданову каноническую форму, уравнение (4.19) получает дополнительные слагаемые, как показывает следующее рассмотрение одного жорданового блока:
$$\begin{align*}
(\dot{\widetilde e}_2(t) \dot{\widetilde e}_3(t) \dots \dot{\widetilde e}_N(t)) = \\
...
Вместо (4.19) для $i = 2$ новым уравнением является
...
Тогда матрицы (4.13) преобразуются в

$${\bar A_i} = A - k{\lambda _i}\{ L\} b{c^{\rm{T}}},\quad i = 2,3, \ldots ,N \tag{4.22}$$

и их можно заставить удовлетворять условию (4.14), соответствующим образом выбрав коэффициент усиления $k$. При введении коэффициента усиления $k$ часто используют нормированную матрицу Лапласа $\hat L$, а не $L$.

![](../../imgs/image005.jpg)

**Рис. 4.5:** Интерпретация условия синхронизации как условия робастного управления
...
Два незатухающих осциллятора с моделью в пространстве состояний (4.2) с
...
Для регулятора (4.21) условие синхронизации требует, чтобы матрица
...
$$\begin{array}{c}22k &gt; 0\quad \Rightarrow \quad k &gt; 0\\9 - 54k &gt; 0\quad \Rightarrow \quad k &lt; 0.167.\end{array}$$
...
**Рис. 4.6:** Корневой годограф агента

На рисунке 4.7 показаны выходные сигналы связанных генераторов для двух выбранных коэффициентов усиления обратной связи. Оба осциллятора стартуют в разных начальных состояниях и приближаются к общей траектории. Как затухает ошибка синхронизации, зависит от собственных значений модели ошибки (4.16)
...
**Рис. 4.7:** Поведение синхронизации генераторов
...
**Требования к структуре связи.** Поскольку матрица $\mathbf{A}$ имеет неустойчивые собственные значения, матрица ${\bar A_i}$ неустойчива при любом обращающемся в нуль собственном значении ${\lambda _i}\{ L\} $, ($i=2,3,\dots,N$). Следовательно, из условия теоремы следует требование

$${\lambda _i}\{ L\} \ne 0,\quad i = 2,3, \ldots ,N,$$

которое выполняется тогда и только тогда, когда коммуникационный граф содержит остовное дерево (ср. с теоремой 2.1).
...
- **Полноценные сети.** Полный граф связи $\vec{\mathcal{G}}$ имеет матрицу Лапласа
...
- **Агенты подключены к одному пути.** Если контроллер использует только связь между парами прямых соседей, как в графе путей на рис. 2.4 на стр. 33, его матрица Лапласа равна
...
- **Топология соединения «звезда».** Если первый агент напрямую связан со всеми остальными, матрица Лапласа
...
Следовательно, взвешенная сумма (4.29) состояний агента движется в соответствии с динамикой неуправляемого агента. Она начинается в исходном состоянии

$${x_{s0}} = \sum\limits_{i = 1}^N {{{\hat \omega }_i}{x_{i0}}.} \tag{4.30} $$
...
выходов агента равна синхронной траектории на всем временном интервале t &#8805; 0. Уравнение (4.32) справедливо и тогда, когда выходы агента ${y_i}(t)$, ($i=1,2,\dots,N$) еще не вышли на общую траекторию.
...
**Рис. 4.8:** Траектории двух осцилляторов с разной структурой связи

**Пример 4.2** *Синхронная траектория двух осцилляторов*

Два осциллятора, представленные в примере 4.1 на стр. 129, теперь связаны двумя разными способами, чтобы показать зависимость синхронной траектории ${y_s}(t)$ от структуры связи. Для двунаправленных связей матрица Лапласа
...
{\begin{array}{*{20}{c}}{0.5}\\{0.5}\end{array}} \right).$$

Следовательно, синхронная траектория генерируется виртуальной эталонной системой

$${\sum _s}:\quad \left\{ \begin{array}{l}{{\dot x}_s}(t) = \left( {\begin{array}{*{20}{c}}0&3\\{ - 3}&0\end{array}} \right){x_s}(t),\quad {x_s}(0) = \dfrac{1}{2}({x_{10}} + {x_{20}})\\{y_s}(t) = \left( {\begin{array}{*{20}{c}}1&{10}\end{array}} \right){x_s}(t).\end{array} \right.$$
...
При изменении структуры связи изменяется исходное состояние виртуальной эталонной системы. В структуре «лидер-последователь» двух осцилляторов лидер определяет начальное состояние синхронной траектории. В примере с осциллятором, ${P_1}$ – в качестве лидера, матрица Лапласа
...
виртуальной эталонной системы. Нижняя часть рис. 4.8 показывает, что теперь второй осциллятор, начавший свое движение в нулевом начальном состоянии, приближается к траектории ведущего агента, которая является синхронной траекторией.  
...
**Сильно связанные агенты.** Обратная связь (4.21) переформулируется как
...
${y_{si}}(t)$ – *локальный опорный сигнал* $i$-го агента, представленный формулой
...
![](../../imgs/image009.jpg)
**Рис. 4.9:** Теоретико-управляющая интерпретация управляемого агента
...
Взаимосвязь контролируемых агентов:
...
используется на рис. 4.9, а $i$-я строка нормированной матрицы смежности $\hat A$ обозначена $\hat a_i^T$.

Уравнения (4.39) и (4.40) можно объединить, чтобы получить модель всей системы $\bar \sum $ аналогично уравнению (4.9) и
...
и $\bar C = {I_N} \otimes {c^{\rm{T}}}$, как и раньше.
...
Звездочки обозначают произвольные элементы из интервала $[-1,0]$. Следовательно, матрица $\hat L$ имеет собственные значения

$$\begin{array}{l}{\lambda _1}\{ \hat L\} = 0\\{\lambda _i}\{ \hat L\} = 1,\quad i = 2,3, \ldots ,N\end{array}$$

и все проверяемые на синхронизацию матрицы ${\bar A_i}$, ($i=2,3,\dots,N$) совпадают с матрицей $\bar A$ управляемых агентов:

$$\begin{array}{c}{{\bar A}_i} = A - k{\lambda _i}\{ \hat L\} b{c^{\rm{T}}}\\ = A - kb{c^{\rm{T}}}\\ = \bar A.\end{array}$$

**Следствие 4.2 (Синхронизация сетей лидер-последователь)**
...
- **Полная сеть:** Матрица Лапласа (4.25) полного графа приводит к нормализованной матрице
...
- **Агенты подключены к одному пути:** Для структуры связи, описываемой нормализованной матрицей Лапласа (4.26), матрица
    $$\bar A = A - kb{c^{\rm{T}}}$$
...
- **Топология соединения «звезда»:** если агенты объединяются в звезду с матрицей Лапласа (4.28), условие синхронизации снова касается матрицы
    $$\bar A = A - kb{c^{\rm{T}}}.$$
...
Эти связи представлены уравнением (4.40) с нормированной матрицей смежности
...
![](../../imgs/image010.jpg)
**Рис. 4.10:** Колонна транспортных средств
...
$$\bar A = \left( {\begin{array}{*{20}{c}}{ - \dfrac{{c + {k_{\rm{P}}}}}{m}}&{\dfrac{{{k_{\rm{I}}}}}{m}}\\{ - 1}&0\end{array}} \right) = \left( {\begin{array}{*{20}{c}}{ - 0.34}&{0.02}\\{ - 1}&0\end{array}} \right).$$
...
![](../../imgs/image012.jpg)
**Рис. 4.12:** Поведение автомобильной колонны ($N$ = 10)
...
**Трансформация модели.** Вся система $\bar \sum $ описывается моделью в пространстве состояний (4.41) с матрицей
...
$${\bar A_a} = A - k\dfrac{N}{{N - 1}}b{c^{\rm{T}}}. \tag{4.53} $$
...
$${\tilde x_{N0}} = \dfrac{1}{N}\sum\limits_{j = 1}^N {{x_{j0}}} = {x_{s0}}. \tag{4.55} $$
...
По сравнению с уравнением (4.52) второй компонент вектора состояния был переименован в ${x_s}(t)$, поскольку соответствующая часть модели читается как
...
$$\widetilde{\sum}_i:\quad \dot{\tilde{x}}_i(t) = \bar{A}_a \tilde{x}_i(t),\quad \tilde{x}_i(0) = \tilde{x}_{i0}.$$
...
![](../../imgs/image013.jpg)
**Рис. 4.13:** Упрощенная модель $i$-го агента в полностью связанной сети
...
порождаемую моделью неуправляемого агента со средним начальным состоянием. Этот результат совпадает с характеристикой виртуальной эталонной системы в теореме 4.2 для используемого здесь графа со сбалансированным весом.
...
если матрица ${\bar A_a}$ асимптотически устойчива, что совпадает с условием, поставленным на матрице (4.47).
...
Если максимальное собственное значение обозначается ${\lambda _{\max }}$, время ${T_{{\rm{sync}}}}$ для синхронизации агентов примерно в пять раз превышает соответствующую постоянную времени:
...
Эти агенты называются *первоначально синхронизированными*, поскольку дальнейшие исследования покажут, что из уравнения (4.61) следует

$${y_i}(t) = \tilde y(t),\quad i \in \bar {\rm N},\;t \ge 0 \tag{4.62} $$

для некоторой общей траектории $\tilde y(t)$.
...
Следовательно, модели (4.56) всех этих агентов имеют одинаковое начальное состояние
...
переходное поведение одинаково, как и общая траектория, из чего следует соотношение (4.62).
